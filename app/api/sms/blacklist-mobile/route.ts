...
export async function GET(request: NextRequest) {
  try {
    ...
    if (token.value.startsWith("demo_token_")) {
      console.log("🎭 使用演示模式拉黑手机号")
      const demoResponse = generateDemoResponse("blacklist_mobile")
    ...
    try {
      console.log("🚫 拉黑手机号参数:", params)
      const data = await makeFailoverRequest(API_ROUTES.BLACKLIST_MOBILE, params)

      console.log("🚫 拉黑手机号响应:", data)
      ...
      return NextResponse.json({ error: data.msg || "拉黑手机号失败" }, { status: 400 })
    } catch (error) {
    ...
    console.error("🔥 Blacklist mobile error:", error)
    return NextResponse.json({ error: "拉黑手机号失败" }, { status: 500 })
  }
}
